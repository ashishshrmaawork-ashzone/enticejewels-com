...
export default function LegacyMedia({ media, fallbackVideo = "/videos/legacy.mp4", title = "Entice Jewels legacy media" }) {
  ...
  return <video key={type === "youtube" ? fallbackVideo : url} autoPlay muted loop playsInline controls className="block aspect-video w-full object-cover"><source src={type === "youtube" ? fallbackVideo : url} />Your browser does not support HTML video.</video>;
}
