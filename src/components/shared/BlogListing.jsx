"use client";
import useLiveListing from "@/lib/useLiveListing";
...
export default function BlogListing({ labels, perPage }) {
  const { data, loading, error, go } = useLiveListing(getBlogsPaged, perPage, "blog-listing");
  const posts = data.items || []; const featured = data.pagination.page === 1 ? data.featured : null;
  return <section id="blog-listing" className="pb-16 md:pb-24 scroll-mt-28"><div className="container mx-auto px-5 sm:px-6 md:px-8">{error && <p role="status" className="text-center mb-5">Unable to refresh posts. Retrying shortly?</p>}
    {loading && <LoaderCircle className="animate-spin text-maroon mx-auto mb-5" />}
    {!loading && !error && !posts.length && !featured ? <p className="text-center text-ink-soft">{labels.empty}</p> : <>{featured && <FeaturedBlog post={featured} labels={labels} />}<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">{posts.map((post) => <BlogCard key={post.slug} post={post} labels={labels} />)}</div></>}
