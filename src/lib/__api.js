...
// Content is managed in WordPress, so GET requests must always reflect the
// latest backend state. Mutating requests retain the browser's normal policy.
const liveFetch = (url, options = {}) => fetch(url, {
  ...options,
...
export async function getGeneralSettings() {
  const response = await liveFetch(`${API_BASE_URL}/settings/general`, { headers: { Accept: "application/json" }, cache: "no-store" });
  if (!response.ok) throw new Error("Unable to load general settings");
  return response.json();
}

// Browser-side refresh used by the global settings provider. Keeping this
// separate preserves static-export caching during the build while ensuring
// recently saved WordPress menus/settings are not hidden by a stale response.
...
export async function getNewsEvents() {
  const response = await liveFetch(`${API_BASE_URL}/news-events`, { headers: { Accept: "application/json" }, cache: "no-store" });
...
export async function getNewsPage() {
  const response = await liveFetch(`${API_BASE_URL}/news-page`, { headers: { Accept: "application/json" }, cache: "no-store" });
...
export async function getNewsEventsPaged(page = 1, perPage = 6) {
  const response = await liveFetch(`${API_BASE_URL}/news-events-paged?page=${page}&per_page=${perPage}`, { headers: { Accept: "application/json" }, cache: "no-store" });
  if (!response.ok) throw new Error(`Unable to load news and events (${response.status})`);
  return response.json();
}

export async function getNewsEvent(slug) {
  const response = await liveFetch(`${API_BASE_URL}/news-events/${encodeURIComponent(slug)}`, { headers: { Accept: "application/json" }, cache: "no-store" });
...
export async function getCareersPage() {
  const response = await liveFetch(`${API_BASE_URL}/careers-page`, { headers: { Accept: "application/json" }, cache: "no-store" });
  if (!response.ok) throw new Error(`Unable to load careers page (${response.status})`);
  return response.json();
}

export async function getBlogPage() { const response = await liveFetch(`${API_BASE_URL}/blog-page`, { headers: { Accept: "application/json" }, cache: "no-store" }); if (!response.ok) throw new Error("Unable to load blog page"); return response.json(); }
export async function getBlogs() { const response = await liveFetch(`${API_BASE_URL}/blogs`, { headers: { Accept: "application/json" }, cache: "no-store" }); if (!response.ok) throw new Error("Unable to load blogs"); return response.json(); }
export async function getBlogsPaged(page = 1, perPage = 6) { const response = await liveFetch(`${API_BASE_URL}/blogs-paged?page=${page}&per_page=${perPage}`, { headers: { Accept: "application/json" }, cache: "no-store" }); if (!response.ok) throw new Error("Unable to load blogs"); return response.json(); }
export async function getBlog(slug) { const response = await liveFetch(`${API_BASE_URL}/blogs/${encodeURIComponent(slug)}`, { headers: { Accept: "application/json" }, cache: "no-store" }); if (!response.ok) return null; return response.json(); }

export async function getJobOpenings() {
  const response = await liveFetch(`${API_BASE_URL}/job-openings`, { headers: { Accept: "application/json" }, cache: "no-store" });
...
export async function getPrivacyPolicyPage() {
  const response = await liveFetch(`${API_BASE_URL}/privacy-policy`, { headers: { Accept: "application/json" }, cache: "no-store" });
