...
// Build-time requests retain their export-compatible options. Browser GETs
// bypass cached responses so hydrated content can reflect backend edits.
const liveFetch = (url, options = {}) => {
  // Keep static generation intact, but refresh browser GETs from WordPress.
  // A unique query also avoids stale intermediary caches keyed by URL.
  if (typeof window !== "undefined" && (options.method || "GET").toUpperCase() === "GET") {
    const freshUrl = new URL(url);
    freshUrl.searchParams.set("_", String(Date.now()));
    return fetch(freshUrl.toString(), { ...options, cache: "no-store", signal: options.signal || AbortSignal.timeout(15000) });
  }
  return fetch(url, options);
};

// ---------------------------------------------------------------------------
// Public content fetchers. Page views call these in the browser and refresh
// every five seconds. Route enumeration and the static sitemap may also call
// them during export; those calls do not supply the live visible page data.
// ---------------------------------------------------------------------------
...
export async function getBlog(slug) { const response = await liveFetch(`${API_BASE_URL}/blogs/${encodeURIComponent(slug)}`, { headers: { Accept: "application/json" } }); if (response.status === 404) return null; if (!response.ok) throw new Error(`Unable to load blog (${response.status})`); return response.json(); }
...
export async function getCollectionCategories(collectionSlug) {
  const response = await liveFetch(`${API_BASE_URL}/${encodeURIComponent(collectionSlug)}/categories`, { headers: { Accept: "application/json" } });
  if (response.status === 404) return null;
