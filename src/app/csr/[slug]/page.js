...
import { getCsrDetail, getCsrItems, getFrontendSeo } from "@/lib/api";
...
async function loadCsrItem(slug) {
  try {
    ...
    console.error("CSR detail API error:", error.message);
    return null;
  }
}
...
export async function generateMetadata({ params }) {
  ...
  const [item, globalSeo] = await Promise.all([loadCsrItem(slug), getFrontendSeo(`/csr/${slug}/`)]);
