...
import { getCareersPage, getFrontendSeo, getJobOpenings } from "@/lib/api";

export async function generateMetadata() {
  const [page, globalSeo] = await Promise.all([getCareersPage().catch(() => null), getFrontendSeo("/careers/")]);
  return { title: page?.seo?.title || "Careers", description: page?.seo?.description || "Build your career at Entice Jewels.", alternates: { canonical: globalSeo.canonical } };
}

export default async function CareersPage() {
  const [page, jobs] = await Promise.all([getCareersPage(), getJobOpenings().catch(() => [])]);
  const icons = [Gem, TrendingUp, Globe, Users];
  const positions = jobs.map((job) => ({ id: job.id, title: job.job_title, experience: job.experience, skills: job.description || job.requirement || job.qualification }));
  return <>
